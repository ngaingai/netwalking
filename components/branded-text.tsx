 ...
 * prefix in brand-teal, suffix in the inherited text color.
 */
  ...
  return (
    <span className={className}>
      {parts.map((part, i) => {
        if (part === "Alex Ngai") {
          return (
            <a key={i} href="https://www.linkedin.com/in/alex-ngai/" target="_blank" rel="noopener noreferrer" className="text-brand-teal hover:underline">
              {part}
            </a>
          );
        }
        ...
        return brand ? (
          <span key={i}>
            <span className="text-brand-teal">{brand[0]}</span>{brand[1]}
          </span>
        ) : (
